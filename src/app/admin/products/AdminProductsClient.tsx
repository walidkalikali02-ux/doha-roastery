...
export default function AdminProductsClient({ initialProducts }: Props) {
  ...
  async function handleDelete(id: string) {
    ...
    if (!err) setProducts(products.filter((p) => p.id !== id))
    setDeleting(null)
  }
  ...
  const F = (key: keyof typeof EMPTY_PRODUCT) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
  ...
  return (
    <div dir={dir} className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={openCreate} className="flex items-center gap-2 bg-charcoal text-cream px-4 py-2.5 text-xs tracking-widest uppercase hover:bg-charcoal-light transition-colors">
          <Plus size={14} />
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-sand/10 bg-cream">
              <th className="text-start px-4 py-3 text-[10px] tracking-widest uppercase text-charcoal/50 font-normal">
                {t('المنتج', 'Product')}
              </th>
              <th className="text-start px-4 py-3 text-[10px] tracking-widest uppercase text-charcoal/50 font-normal">
                {t('الفئة', 'Category')}
          ...
          <tbody className="divide-y divide-sand/10">
            {filtered.length === 0 && (
              <tr>
                <td colSpan={6} className="text-center py-12 text-charcoal/30 tracking-widest text-xs">
              ...
              <tr key={p.id} className="hover:bg-cream/50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2 justify-end">
                    <button onClick={() => openEdit(p)} className="text-charcoal/40 hover:text-sand transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-charcoal/60 p-4">
          <div className="bg-white w-full max-w-2xl max-h-[90vh] overflow-y-auto" dir={dir}>
            ...
            <div className="p-6 grid grid-cols-2 gap-5">
              ...
              {error && (
                <div className="col-span-2 text-red-500 text-xs tracking-wide">{error}</div>
              )}
            </div>

            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-sand/20 sticky bottom-0 bg-white">
              <button onClick={() => setModal(null)} className="text-xs tracking-widest uppercase border border-sand/40 px-5 py-2.5 text-charcoal hover:border-charcoal transition-colors">
                {t('إلغاء', 'Cancel')}
              </button>
              <button
                onClick={handleSave}
